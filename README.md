...
### BLiSS Causal Innovations (Extended)

This repository extends the original A2Summ with **causal learning innovations** for the BLiSS dataset:

| Innovation | Module | Description |
|:-----------|:-------|:------------|
| Causal Contrastive Loss | `causal_contrastive_loss.py` | Learn to distinguish core content via counterfactual interventions |
| Intervention Dissimilarity | `causal_alignment_discovery.py` | Unsupervised cross-modal alignment discovery |

For detailed module documentation, see [docs/modules.md](docs/modules.md).  
For BLiSS experiment configurations, see [configs/bliss_experiments.yaml](configs/bliss_experiments.yaml).

**Example: Train BLiSS with full causal innovations**
```bash
