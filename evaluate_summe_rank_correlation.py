"""
SumMe Rank Correlation Evaluation Script
=========================================
Evaluates the Spearman and Kendall rank correlation coefficients for a trained SumMe model.
...
Calculated by comparing the predicted score profile (upsampled to full frame rate) 
against the continuous ground truth importance scores.

Note: SumMe uses 'max' evaluation metric (best user score) vs TVSum uses 'avg'.
"""
...
def evaluate_rank_correlation(args):
    ...
    print(f"Eval Mode: MAX (SumMe uses best user score)")
    print(f"{'='*60}")
    ...
    for split_idx in range(num_splits):
        ...
        with torch.no_grad():
            for key in tqdm(test_keys, desc=f"  Evaluating", leave=False):
                ...
                # ==== KEY: Load CONTINUOUS gtscore ====
                gtscore = video_file['gtscore'][...].astype(np.float32)  # [T] continuous
                gtscore_normalized = (gtscore - gtscore.min()) / (gtscore.max() - gtscore.min() + 1e-8)
                
                # Upsample gtscore to full frame rate
                gt_frame_scores = upsample_scores_to_full_frames(gtscore_normalized, picks, n_frames)
                ...
                # Upsample predictions to full frame rate
                pred_frame_scores = upsample_scores_to_full_frames(pred_scores, picks, n_frames)
                
                # ==== Compute rank correlation with CONTINUOUS gtscore ====
                min_len = min(len(pred_frame_scores), len(gt_frame_scores))
                pred_s = pred_frame_scores[:min_len]
                gt_s = gt_frame_scores[:min_len]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='SumMe Rank Correlation Evaluation')
