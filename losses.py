...
def calc_ctr_loss(pred_batch, test_batch, pos_mask):
    ...
    valid_samples = 0
    for i in range(batch_size):
        # Align lengths if mismatched (robustness for length variations)
        curr_len = min(pred_batch.shape[1], pos_mask.shape[1], test_batch.shape[1])
        
        # Use sliced views
        curr_pos_mask = pos_mask[i, :curr_len]
        curr_pred_batch = pred_batch[i, :curr_len]
        curr_test_batch = test_batch[i, :curr_len]
        
        pred = curr_pred_batch[curr_pos_mask] #[M]
        test = curr_test_batch[curr_pos_mask] #[M]
        
        if pred.numel() > 0:
            # Clamp to prevent Log(0) which causes CUDA assert
    ...
    if valid_samples > 0:
        ...
        loss = torch.tensor(0.0, device=pred_batch.device, requires_grad=True)
