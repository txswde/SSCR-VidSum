...
class BLiSSDataset(object):
    def __init__(self, mode='train', args=None):
        ...
        # Subsample training data for ablation experiments
        data_ratio = getattr(args, 'data_ratio', 1.0)
        if mode == 'train' and data_ratio < 1.0:
            n_total = len(self.clip_id_list)
            n_keep = max(1, int(n_total * data_ratio))
            rng = random.Random(getattr(args, 'seed', 12345))
            self.clip_id_list = sorted(rng.sample(self.clip_id_list, n_keep))
            print(f'[BLiSSDataset] data_ratio={data_ratio}: using {n_keep}/{n_total} training samples')
...
class VideoSummDataset(object):
    def __init__(self, keys, args=None):
        ...
        cf_strategy = getattr(args, 'cf_strategy', 'mixed')
        self.cf_generator = CounterfactualGenerator(strategy=cf_strategy, random_seed=42)
    ...
    def __getitem__(self, index):
        ...
        # === Causal Intervention: Multi-level Counterfactual Generation ===
        # Generate both minimal and severe counterfactual interventions
        text_cf_minimal, text_cf_severe = self.cf_generator.generate_triplet(
            text_features=text,
            text_id=f"{video_name}_{index}"
        )
        ...
        return video, text, mask_video, mask_text, video_cls_label, video_loc_label, video_ctr_label, text_cls_label, text_loc_label, text_ctr_label, \
            user_summary, n_frames, ratio, n_frame_per_seg, picks, change_points, video_to_text_mask, text_to_video_mask, text_cf_minimal, text_cf_severe
