...
class Model_MSMO(nn.Module):
    ...
    def select_contrastive_embedding(self, score, embedding, mask, label):
        ...
        for i in range(B):
            ...
            key_embedding_num = max(1, torch.div(length, self.ratio, rounding_mode='trunc').item())
            nonkey_embedding_num = max(1, torch.div(length, self.ratio, rounding_mode='trunc').item())
...
class Model_VideoSumm(nn.Module):
    def __init__(self, args=None):
        ...
        # Separate causal features from spurious correlations
        self.causal_feature_head = nn.Linear(num_hidden, num_hidden // 2)
        self.spurious_feature_head = nn.Linear(num_hidden, num_hidden // 2)
        
        # === Differentiable Causal Attribution Layer (Innovation 3) ===
    ...
    def forward(self, **kwargs):
        ...
        # === Causal Branch Input Processing: Triplet Counterfactuals ===
        text_cf_minimal = kwargs.get('text_cf_minimal', None)
        text_cf_severe = kwargs.get('text_cf_severe', None)
        
        if text_cf_minimal is not None:
            text_cf_minimal = self.proj_fc_text(text_cf_minimal)
            text_cf_minimal = torch.cat([self.cls_token_text.expand(B, -1, -1), text_cf_minimal], dim=1)
            text_cf_minimal = text_cf_minimal + self.pos_embed_text[:, :N_text, :] + self.type_text + self.pos_embed_segment[:, :N_text, :]
        
        if text_cf_severe is not None:
            text_cf_severe = self.proj_fc_text(text_cf_severe)
            text_cf_severe = torch.cat([self.cls_token_text.expand(B, -1, -1), text_cf_severe], dim=1)
            text_cf_severe = text_cf_severe + self.pos_embed_text[:, :N_text, :] + self.type_text + self.pos_embed_segment[:, :N_text, :]
        ...
        # N_video_valid: number of actual video frames for each data sample
        # N_text_valid: number of actual text frames for each data sample
        mask_fused = torch.zeros((B, N_video+N_text, N_video+N_text), dtype=torch.long, device=mask_video.device) # [B, N_video+N_text, N_video+N_text]
        for i in range(B):
            mask_fused[i, :N_video, :N_video] = mask_video[i].view(1, N_video).expand(N_video, -1) #[N_video, N_video]
            mask_fused[i, N_video:, N_video:] = mask_text[i].view(1, N_text).expand(N_text, -1) #[N_text, N_text]
            N_video_valid, N_text_valid = video_to_text_mask_list[i].shape #[N_video_valid, N_text_valid]
        ...
        # === 1. Main Branch Forward ===
        # multiway transformer layers
        fused = torch.cat([video, text], dim=1)
        
        # Keep references for residual connection later
        # AND IMPORTANT: Keep reference to video/text with embeddings for Causal Branch (before they get updated/split)
        video_with_embed = video 
        text_with_embed = text  # For no-text branch
        ...
        causal_feat = self.causal_feature_head(text)  # [B, T, D//2]
        spurious_feat = self.spurious_feature_head(text)  # [B, T, D//2]
        
        # === 3. Causal Branch Forward: Triplet Counterfactuals ===
        pred_video_cls_cf_minimal = None
        pred_video_cls_cf_severe = None
        text_feat_cf_min = None
        text_feat_cf_sev = None
        
        # Process minimal counterfactual
        if text_cf_minimal is not None:
            curr_video_cf = video_with_embed 
            curr_text_cf = text_cf_minimal
            
            for i in range(self.num_layers):
                curr_fused_cf = torch.cat([curr_video_cf, curr_text_cf], dim=1)
                curr_video_cf, curr_text_cf = self.multiway_list[i](curr_fused_cf, mask_fused, N_video, N_text)
            
            _, video_content_cf_min = torch.split(curr_video_cf, [1, N_video-1], dim=1)
            _, text_content_cf_min = torch.split(curr_text_cf, [1, N_text-1], dim=1)
            
            video_out_cf_min = self.norm_video(residual_video + video_content_cf_min)
            text_feat_cf_min = self.norm_text(residual_text + text_content_cf_min)
            text_feat_cf_min = self.fc_text(text_feat_cf_min)
            
            video_feat_cf_min = self.fc_video(video_out_cf_min)
            pred_video_cls_cf_minimal = self.fc_video_cls(video_feat_cf_min).squeeze(-1)
        
        # Process severe counterfactual
        if text_cf_severe is not None:
            curr_video_cf = video_with_embed 
            curr_text_cf = text_cf_severe
            
            for i in range(self.num_layers):
                curr_fused_cf = torch.cat([curr_video_cf, curr_text_cf], dim=1)
                curr_video_cf, curr_text_cf = self.multiway_list[i](curr_fused_cf, mask_fused, N_video, N_text)
            
            _, video_content_cf_sev = torch.split(curr_video_cf, [1, N_video-1], dim=1)
            _, text_content_cf_sev = torch.split(curr_text_cf, [1, N_text-1], dim=1)
            
            video_out_cf_sev = self.norm_video(residual_video + video_content_cf_sev)
            text_feat_cf_sev = self.norm_text(residual_text + text_content_cf_sev)
            text_feat_cf_sev = self.fc_text(text_feat_cf_sev)
            
            video_feat_cf_sev = self.fc_video(video_out_cf_sev)
            pred_video_cls_cf_severe = self.fc_video_cls(video_feat_cf_sev).squeeze(-1)
        
        # === NEW: No-Text Branch for Text Necessity Loss ===
        # REMOVED: Redundant computation, simplified loss does not use this.
        pred_video_cls_no_text = None
        
        # === 5. Differentiable Causal Attribution (Innovation 3) ===
        
        # === 6. Return Comprehensive Outputs ===
        return {
            # Original predictions
            'video_pred_cls': pred_video_cls,
            'video_pred_loc': pred_video_loc,
            'video_pred_ctr': pred_video_ctr,
            'text_pred_cls': pred_text_cls,
            'text_pred_loc': pred_text_loc,
            'text_pred_ctr': pred_text_ctr,
            'contrastive_pairs': contrastive_pairs,
            # Counterfactual predictions
            'video_pred_cls_cf_min': pred_video_cls_cf_minimal,
            'video_pred_cls_cf_sev': pred_video_cls_cf_severe,
            # No-text prediction for text necessity loss
            'video_pred_cls_no_text': pred_video_cls_no_text,
            # Features for causal contrastive loss
            'video_feat': video,  # Added for alignment loss
            'text_feat_orig': text,
            'text_feat_cf_min': text_feat_cf_min,
            'text_feat_cf_sev': text_feat_cf_sev,
            # Disentangled features
            'causal_feat': causal_feat,
            'spurious_feat': spurious_feat,
            # Differentiable causal attribution output (Innovation 3) removed
        }

    def predict(self, **kwargs):
        outputs = self(**kwargs)
        pred_video_cls = outputs['video_pred_cls']
        pred_video_loc = outputs['video_pred_loc']
        pred_video_ctr = outputs['video_pred_ctr']
